...
`amount` is the swap amount you want executed into the pool. `quote.outputAmount` is calculated from `quote.inputAmountAfterPoolFee`, which is the swap amount after deducting the 0.1% swap fee. OneSwap then adds `quote.networkFeeAmount` on top to produce `quote.totalInputAmount`, the exact deposit amount the user needs to send. `quote.trafficFeeInInput` remains available as a compatibility alias for `quote.networkFeeAmount`.
...
console.log(swap.quotedSwapAmount) // the swap amount you asked OneSwap to price
