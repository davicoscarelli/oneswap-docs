...
That resolves to `https://devnet.api.oneswap.cc` by default.
...
`quote.outputAmount` is already calculated from `quote.inputAmountAfterTrafficFee`. The traffic recovery is separate from the 0.1% swap fee.
