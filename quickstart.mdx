...
console.log(swap.depositReference) // include in Canton reason/reference field
...
The user sends tokens from `partyId` directly to `swap.depositAddress` and includes `swap.depositReference` in the Canton reason/reference field. Do not send from a different wallet or Canton party, or the swap will not complete.
