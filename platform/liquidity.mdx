...
<Steps>
  <Step title="Select a Pool">
    Choose which token pair you want to provide liquidity for (for example `CC (Amulet)` / `USDCx`). You can see current reserves, fee earnings, and APR for each pool.
  </Step>
  ...
  <Step title="Send Both Tokens">
    Transfer both token amounts from the same party to the LP address. You currently have **24 hours** to complete both deposits.
  </Step>
...
After 1,000 swaps generating fees, your 1% share now represents more `CC (Amulet)` and `USDCx` than you originally deposited.
...
pending → partial (one token received) → processing (both received) → completed (LP position credited)
        ...
        → expired (no deposits before the current 24-hour deadline)
```
