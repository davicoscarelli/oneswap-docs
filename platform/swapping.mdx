...
Every swap incurs a **2% fee** taken from the input amount. This fee is split between liquidity providers (1.6%), the platform (0.4%), and — for SDK swaps — the integrating developer (0.2% from the platform's share).

| Fee | Amount | Goes To |
|-----|--------|---------|
| LP fee | 1.6% | Liquidity providers |
| Platform fee | 0.4% | OneSwap (0.2% platform + 0.2% developer for SDK swaps) |
...
<Info>
Most quote and liquidity failures are refunded automatically. Rare operational states such as `no_output_holdings` or `refund_failed` require manual investigation by OneSwap.
</Info>
