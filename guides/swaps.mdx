...
| `quotedSwapAmount` | Original swap amount you asked OneSwap to quote into the pool |
| `networkFeeAmount` | Estimated network fee added on top of the swap amount |
| `poolFeeAmount` | The 0.1% swap fee deducted before pricing into the pool |
...
- `inputAmount` is the swap amount you asked OneSwap to price into the pool.
- `inputAmountAfterPoolFee` is the amount that actually trades into the pool after deducting the 0.1% swap fee.
- `outputAmount` is quoted from `inputAmountAfterPoolFee`, not from `totalInputAmount`.
- `networkFeeAmount` is the estimated Canton network fee added on top of the swap amount.
- `totalInputAmount` is the exact deposit amount to send if you create the intent.
...
- The network fee is separate from the 0.1% swap fee and is not shared with LPs or SDK developers.
