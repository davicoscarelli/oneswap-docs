...
Cancel a pending or partial LP intent:

```typescript
await client.liquidity.cancel(lp.poolId, lp.intentId)
```

Only `pending` and `partial` intents can be cancelled. Once both tokens are received (`processing`), the intent cannot be cancelled.

## Status lifecycle

```
pending → partial (one token received)
        → processing (both received, forwarding)
        → completed (LP minted)
