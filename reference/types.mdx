...
interface Quote {
  ...
  poolFeeAmount?: string          // absolute 0.1% swap fee deducted before AMM pricing
  lpFeeAmount?: string
...
`inputAmount` is the exact deposit amount you want the user to send. OneSwap deducts the estimated network fee internally from that deposit, then deducts the 0.1% pool fee from the remainder before pricing the output. `totalInputAmount` currently echoes that exact deposit amount, and `outputAmount` is priced from `inputAmountAfterPoolFee`.
