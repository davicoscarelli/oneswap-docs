...
interface Quote {
  ...
  inputAmount: string             // swap amount entered by the user
  totalInputAmount?: string       // final deposit amount after adding the network fee
  outputAmount: string            // quoted from inputAmountAfterPoolFee
  rate: string
  priceImpact: string
  fee: number                     // swap fee rate, e.g. 0.001 for 0.1%
  poolFeeAmount?: string          // absolute 0.1% swap fee in the input token
  lpFeeAmount?: string
  platformFeeAmount?: string
  inputAmountAfterPoolFee?: string
  networkFeeAmount?: string       // estimated network fee added on top of the swap amount
  poolId: string
...
`inputAmount` is the amount you want swapped into the pool. `totalInputAmount` is the larger deposit you actually send after adding the estimated network fee. `outputAmount` is priced from `inputAmountAfterPoolFee`, not from `totalInputAmount`.
...
In `SwapIntentResponse`, `expectedAmount` is the final deposit amount to send. `quotedSwapAmount` is the original swap amount you asked OneSwap to price, and `networkFeeAmount` is the estimated network fee added on top.
