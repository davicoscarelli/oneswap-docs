...
interface LpStatusResponse {
  intentId: string
  poolId: string
  status: 'pending' | 'partial' | 'processing' | 'completed' | 'refunded' | 'failed' | 'expired' | 'cancelled'
  tokenA: string
  tokenB: string
  expectedAmountA: string
  expectedAmountB: string
...
class LpRefundedError extends OneSwapError {}  // LP intent refunded
class LpFailedError extends OneSwapError {}    // LP intent failed
