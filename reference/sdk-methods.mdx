...
| Method | Description |
|---|---|
| `client.quotes.get({ from, to, amount, receiverParty?, poolId?, fromAdmin?, toAdmin? })` | Price quote. `amount` is the swap amount you want priced into the pool. The response includes `inputAmountAfterPoolFee`, `networkFeeAmount`, and `totalInputAmount`. `outputAmount` is based on `inputAmountAfterPoolFee`, and `totalInputAmount` is the exact deposit amount to send after adding the estimated network fee. `trafficFeeInInput` remains as a compatibility alias for `networkFeeAmount`. When token symbols are reused across issuers, pass `poolId` or both admin fields to disambiguate the pool pair. |
...
- `quotedSwapAmount` is the swap amount you entered before fees.
- `poolFeeAmount` is the 0.1% swap fee taken from `quotedSwapAmount` before pricing.
- `networkFeeAmount` is the estimated network fee added on top of the swap amount to reach the final deposit. `trafficFeeInInput` remains as a compatibility alias.
