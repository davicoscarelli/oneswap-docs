...
5. OneSwap returns the pool swap party as the `depositAddress`
6. The user sends tokens directly from that same party to the pool party
...
Create your `os_live_...` API key in the wallet-authenticated OneSwap developer portal. Legacy `POST /api/sdk/register` and `POST /api/sdk/login` are retired and return `410 Gone`.
